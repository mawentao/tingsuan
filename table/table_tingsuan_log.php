 ...
 * 插件访问日志表
 **/
class table_tingsuan_log extends discuz_table
{
	public function __construct() {
		$this->_table = 'tingsuan_log';
		$this->_pk = 'logid';
		parent::__construct();
	}
	...
	{/*{{{*/
		...
		$sday  = tingsuan_validate::getNCParameter('sday','sday','string'); 
		$eday  = tingsuan_validate::getNCParameter('eday','eday','string'); 
		$key   = tingsuan_validate::getNCParameter('key','key','string'); 
        $sort  = tingsuan_validate::getOPParameter('sort','sort','string',1024,'logtime');
		...
		$table_tingsuan_log = DB::table($this->_table);
		$table_common_member = DB::table('common_member');
		$sql = <<<EOF
SELECT SQL_CALC_FOUND_ROWS a.*,b.username,b.email
FROM $table_tingsuan_log as a LEFT JOIN $table_common_member as b ON a.uid=b.uid
...
FROM $table_tingsuan_log WHERE $where GROUP BY vday ORDER BY logtime ASC
EOF;
