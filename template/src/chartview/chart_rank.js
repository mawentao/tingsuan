define(function(require){
	...
	o.show=function(domid,data,cubeid) {
		...
		if (dims.length!=1) throw new Error('请选择1个维度');
		...
		for (var k in piemap) {
			...
			var pim = {
				name: metricInfo.mname, //diminfo.dimname+'分布',
				data: []
			};
			var mp = piemap[k];
			...
			var maxPieNum = 20;  //!< 最多显示10个组成,超出部分合并为其他
...
//        console.log(data);
        var metric = data[0].name;
        var yData = [];
        var serieData = [];
        for (var i=data[0].data.length-1;i>=0;--i) {
            var im = data[0].data[i];
            yData.push(im.name);
            var sim = {
                value: im.value
            };
            if (i<3) {
                sim['itemStyle'] = {color:'#FFB84F'};
            }
            serieData.push(sim);
        }
