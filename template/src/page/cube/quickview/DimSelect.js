...
var DimSelect=function(opt)
{
	...
	{/*{{{*/
		...
		var code = '<div id="'+gridid+'" class="mwt-layout-fill" style="background:#fff;"></div>';
...
//				{html:'<label style="color:#0096A9;">选择'+dimInfo.dimname+'</label>'},'->',
              	{type:"search",id:"sokey-"+optdivid,width:140,placeholder:'查询关键词',handler:query},
                '->',
	...
	{/*{{{*/
		store.baseParams = {
            key: mwt.get_value('sokey-'+optdivid),
	...
	this.create = function() {
		...
		var code = '<div>'+
           	'<a id="'+btnid+'" style="width:150px;" '+
                'class="mwt-btn mwt-btn-default mwt-btn-xs mwt-field dimmsel" href="javascript:;">'+
              '<span id="'+txtid+'" class="dimlabel" style="max-width:100px">请选择...</span><i class="'+icon+'"></i>'+
            '</a>'+
          '</div>'+
        '</div>';
		...
		jQuery("#"+btnid).click(function(event){
			if (!mwt.$(optdivid)) {
				MWT.createDiv(optdivid);
				var dom=MWT.$(optdivid);
                var position = getPopPosition(btnid,510,360);
				dom.style = "z-index:99999;position:absolute;"+position+"border:solid 1px #ccc;background:#fff;box-shadow:0 0 20px #ccc;";
    ...
    // 获取弹出层位置
    function getPopPosition(anchor,width,height) 
    {/*{{{*/
        var pos = {top:80,left:0,right:0,height:height};
/*
        //1. 获取anchor位置
        ...
        if (bodyHeight>height && pos.top+height>bodyHeight) {
            pos.top = a.y-height-2;
        }
*/
        var posStyle = "";
        for (var k in pos) {
